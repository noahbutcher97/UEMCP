...

{
  ...
  const available = await conn.isLayerAvailable('tcp-55558', true);
  t.assert(available === true, 'tcp-55558 reports available via fake ping');
  ...
  t.assert(fake.calls[0].port === 55558, `call port is 55558 (got ${fake.calls[0].port})`);
}
...

{
  ...
  const result1 = await conn.send('tcp-55558', 'list_actors', {});
  ...
  const result2 = await conn.send('tcp-55558', 'list_actors', {});
  ...
  const result3 = await conn.send('tcp-55558', 'list_actors', {}, { skipCache: true });
...

{
  ...
  const r1 = await conn.send('tcp-55558', 'get_counter', { msg: 'hello' }, { skipCache: true });
  t.assert(r1.count === 1, `first call count=1 (got ${r1.count})`);
  t.assert(r1.echo === 'hello', 'factory receives params');

  const r2 = await conn.send('tcp-55558', 'get_counter', { msg: 'world' }, { skipCache: true });
...

{
  ...
  const r1 = await conn.send('tcp-55558', 'get_data', {}, { skipCache: true });
  r1.items.push('MUTATED');

  const r2 = await conn.send('tcp-55558', 'get_data', {}, { skipCache: true });
...

{
  ...
  await t.assertRejects(
    () => conn.send('tcp-55558', 'bad_cmd', {}, { skipCache: true }),
    'Actor not found',
...

{
  ...
  await t.assertRejects(
    () => conn.send('tcp-55558', 'bad_cmd2', {}, { skipCache: true }),
    'Permission denied',
...

{
  ...
  await t.assertRejects(
    () => conn.send('tcp-55558', 'list_actors', {}, { skipCache: true }),
    /timeout/i,
...

{
  ...
  await t.assertRejects(
    () => conn.send('tcp-55558', 'ping', {}, { skipCache: true }),
    /ECONNREFUSED/i,
...

{
  ...
  await t.assertRejects(
    () => conn.send('tcp-55558', 'any_cmd', {}, { skipCache: true }),
    /simulated error/i,
...

{
  ...
  t.assert(statusBefore['tcp-55558'].status === 'unknown', 'initial status is unknown');

  // After successful probe
  await conn.isLayerAvailable('tcp-55558', true);
  const statusAfter = conn.getStatus();
  t.assert(statusAfter['tcp-55558'].status === 'available', 'status is available after ping');
  ...
  await conn2.isLayerAvailable('tcp-55558', true);
  const statusFail = conn2.getStatus();
  t.assert(statusFail['tcp-55558'].status === 'unavailable', 'status is unavailable after timeout');
  t.assert(statusFail['tcp-55558'].error !== null, 'error message preserved');
}
...

{
  ...
  const [r1, r2] = await Promise.all([
    conn.send('tcp-55558', 'slow_cmd', { id: 1 }, { skipCache: true }),
    conn.send('tcp-55558', 'slow_cmd', { id: 2 }, { skipCache: true }),
  ]);
...

{
  ...
  await t.assertRejects(
    () => conn.send('tcp-55558', 'unknown_cmd', {}, { skipCache: true }),
...

{
  ...
  await handler(55558, 'ping', {}, 5000);
  await handler(55558, 'list_actors', { filter: 'a' }, 5000);
  await handler(55558, 'ping', {}, 5000);
  ...
  const r = await handler(55558, 'ping', {}, 5000);
...

{
  ...
  const handler = fake.handler();
  const r = await handler(55558, 'anything', {}, 5000);
  ...
  const r2 = await handler(55558, 'specific', {}, 5000);
...

{
  ...
  agg.record({ port: 55558, type: 'list_actors', ok: false, err: 'timeout', framed: false, total_ms: 5000 });
...

{
  ...
  await conn.send('tcp-55558', 'list_actors', {});
...
// ── Test 25: active layer status/probe excludes retired layer ──
