...
try {
  ...
  assert(/AnimBlueprint|AnimGraph|anim_graph/i.test(offlineTools.bp_list_graphs.description),
    'D184: bp_list_graphs yaml mentions AnimBlueprint AnimGraph coverage');
  ...
  assert(/pin_defaults|default_value|default_object|default_text_value|autogenerated_default_value/i.test(offlineTools.bp_show_node.description),
    'D184: bp_show_node yaml mentions pin default literal fields');
} catch (e) {
...
if (!HAS_REAL_ASSETS) {
  ...
  try {
    ...
    const animRel = ANIM_BLUEPRINT_BP.path.replace(/^\/Game\//, 'Content/') + '.uasset';
    const hasAnimFixture = await stat(join(PROJECT_ROOT, animRel)).then(() => true, () => false);
    if (hasAnimFixture) {
      const anim = await executeOfflineTool('bp_list_graphs', { asset_path: ANIM_BLUEPRINT_BP.path }, PROJECT_ROOT);
      const animGraph = anim.graphs.find(g => g.name === 'AnimGraph');
      assert(animGraph?.graph_type === 'anim_graph',
        'D184: bp_list_graphs includes AnimBlueprint AnimGraph as graph_type=anim_graph');
      assert(animGraph?.node_count > 0,
        'D184: AnimGraph row counts AnimGraphNode_* exports');
    } else {
      console.log('  SKIP: AnimBlueprint graph fixture not found — skipping D184 AnimGraph check');
    }
  } catch (e) {
