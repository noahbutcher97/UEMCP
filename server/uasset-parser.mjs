...
// FUE5ReleaseStreamObjectVersion's GUID, byte-swapped per uint32:
// D89B5E42-24BD4D46-8412ACA8-DF641779.
const UE5_RELEASE_VERSION_KEY = '425e9bd8464dbd24a8ac1284791764df';

// FUE5ReleaseStreamObjectVersion::SerializeFloatPinDefaultValuesAsSinglePrecision
// — the version that added the trailing int32 to FEdGraphPinType.
// UE5ReleaseStreamObjectVersions.inl:118.
const SINGLE_PRECISION_FLOAT_PIN_DEFAULTS_VERSION = 36;
...
export function pinBlockLayoutForPackage(summary) {
  const versions = summary?.customVersions ?? [];
  const versionFor = key => {
    const entry = versions.find(v => v.key === key);
    return Number.isFinite(entry?.version) ? entry.version : -1;
  };
  return {
    hasSourceIndex: versionFor(UE5_MAINSTREAM_VERSION_KEY) >= EDGRAPH_PIN_SOURCE_INDEX_VERSION,
    // FEdGraphPinType's trailing bSerializeAsSinglePrecisionFloat. Assuming a
    // fixed-size pin type overshoots by 4 bytes on any package below this,
    // desyncing from pin 0 onward.
    hasSinglePrecisionFloatPinDefaults:
      versionFor(UE5_RELEASE_VERSION_KEY) >= SINGLE_PRECISION_FLOAT_PIN_DEFAULTS_VERSION,
  };
}
...
export function parsePinBlock(buf, exportEntry, names, opts = {}) {
  ...
  for (let i = 0; i < arrayCount; i++) {
    ...
    try {
      pins.push(readPinBody(cur, names, exportEnd, opts));
    } catch (err) {
      blockMalformed = true;
 ...
 *   autogenerated_default_value?: string,
 *   default_object?: object,
 *   default_text_value?: string,
 *   malformed?: boolean
 * }}
 */
function readPinBody(cur, names, exportEnd, opts = {}) {
  ...
  // FEdGraphPinType.
  skipEdGraphPinType(cur, opts);
  // FString DefaultValue.
  const defaultValue = cur.readFString();
  // FString AutogeneratedDefaultValue.
  const autogeneratedDefaultValue = cur.readFString();
  // int32 DefaultObject FPackageIndex.
  const defaultObjectIndex = cur.readInt32();
  let defaultObject = null;
  if (defaultObjectIndex !== 0) {
    defaultObject = opts.resolve?.(defaultObjectIndex) ?? { packageIndex: defaultObjectIndex };
  }
  // FText DefaultTextValue.
  const defaultTextValue = readFText(cur);
  ...
  const out = { pin_id, name: pin_name, direction, linked_to_raw, bOrphanedPin };
  if (defaultValue !== '') out.default_value = defaultValue;
  if (autogeneratedDefaultValue !== '') out.autogenerated_default_value = autogeneratedDefaultValue;
  if (defaultObject) out.default_object = defaultObject;
  if (defaultTextValue !== '') out.default_text_value = defaultTextValue;
  return out;
}
...
/**
 * Read an FText value. Layout (UE 5.6, all custom versions latest):
 *   int32 Flags
 *   int8 HistoryType
 ...
 * @param {Cursor} cur
 */
function skipEdGraphPinType(cur, opts = {}) {
  ...
  cur.skip(4 + 4); // bIsConst + bIsUObjectWrapper
  // bSerializeAsSinglePrecisionFloat is gated on FUE5ReleaseStreamObjectVersion
  // >= SerializeFloatPinDefaultValuesAsSinglePrecision. Skipping it
  // unconditionally overshoots by 4 bytes on older packages, which desyncs the
  // pin array from its very first entry.
  if (opts.hasSinglePrecisionFloatPinDefaults !== false) cur.skip(4);
}
...
export function resolveLinkedToEdges(buf, exports, imports, names, opts = {}) {
  ...
  for (const node of parsedByExportIdx.values()) {
    ...
    for (const p of node.pins) {
      ...
      if ('default_value' in p) pinsOut[p.pin_id].default_value = p.default_value;
      if ('autogenerated_default_value' in p) {
        pinsOut[p.pin_id].autogenerated_default_value = p.autogenerated_default_value;
      }
      if ('default_object' in p) pinsOut[p.pin_id].default_object = p.default_object;
      if ('default_text_value' in p) pinsOut[p.pin_id].default_text_value = p.default_text_value;
    }
