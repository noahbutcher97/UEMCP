// descriptor.mjs — builds and compares the canonical MCP server descriptor written into every client's config.
// Why: every adapter writes the same six-key descriptor (`DESCRIPTOR_KEYS`:
// name, transport, command, args, env, cwd) pointing at this Node executable
// and server entry; built from fingerprinted, non-linked files and compared
// for path-normalized equality so a "no-op" plan is trustworthy.
