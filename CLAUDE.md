...
7. **Transport choice — bright-line rule (D127 / D128)** — **all UEMCP tools ship on TCP:55558** (UEMCP plugin C++ handler). The RC HTTP transport (`http-30010`) is being retired in full per Goal State B (D128 commitment); workstream covers delegate migrations + primitives reimplementation + final RC dep removal. **Implementation-style distinction (NOT transport distinction)**: productized delegate tools (fixed schema, single-purpose, agent-facing semantic surface) call specific UE editor APIs directly from C++ handlers. Reflection-by-name tools (`rc_get_property` / `rc_set_property` / `rc_call_function` / `rc_describe_object` / `rc_list_objects` / `rc_batch`) walk UE's native reflection APIs (`FProperty`, `UFunction`, `UStruct`, `TFieldIterator`) from C++ handlers — same TCP transport, different internal implementation pattern. **Rationale for full retirement (post-D128)**: RC was the source of 100% of UEMCP's shipped-tool reliability bugs (NEW-2 batch race, NEW-4 struct-param coercion); even the 8 "always-clean" primitives carried foot-gun risk and didn't address the silent-fail UX gap of an "optional RC" model. Migration cost is bounded (~5 worker sessions) and aligns with `feedback_self_contained_scope.md`. **D66 / D74 / D76 / D101 (ii) walked-back**: the original HYBRID transport rationale ("save us from writing C++") was net-negative once lifetime debugging cost was counted — see D128's institutional-memory note. **Future tool development**: when adding a new tool, ask "is this productized or reflection-by-name?" — productized tools wrap a specific UE API in a C++ handler; reflection-by-name tools route through the `ReflectionPrimitives` module (post-primitives-reimplementation). Both are TCP. RC HTTP is not a UEMCP transport.
...
### WebRemoteControl operational limits (D120 / D122 / D125 / D128 / NEW-2)

> **Status (D128, 2026-05-03)**: Goal State B committed — UEMCP is retiring the RC plugin entirely. The §below captures operational constraints during the transitional period (delegates + primitives still partially on RC pending migration). Once the workstream lands (`rc-to-tcp-poc-set-material-parameter.md` + `rc-to-tcp-bundle-material-mesh.md` + `rc-to-tcp-primitives-reimplementation.md` + `rc-dep-removal-cleanup.md`), this entire section is removed by the cleanup worker.

#### `/remote/batch` crash trigger (transitional — only relevant pre-migration)
