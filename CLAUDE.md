...
### `get_mesh_info` status (Pivot-W3 source-side verified, D139)
...
**Pivot-W3 verdict (D139, 2026-05-06)**: source-side verified clean. UE 5.6 `StaticMesh.h` confirms all 5 target methods are UFUNCTION-decorated (`UFUNCTION(BlueprintPure)` on the 4 scalar getters; `UFUNCTION(BlueprintGetter)` on `GetStaticMaterials`). D131's Passphrase header fix in `connection-manager.mjs httpHeaders()` eliminates the `/remote/batch` engine crash class. Tools.yaml description already reflects D131 fix-shipped status. No code change required; existing dispatch shape (`/remote/batch` with 5 sub-requests) is correct post-D131 and saves 4 round-trips vs per-call migration. **Live-fire confirmation pending one Noah-fired call** against any UStaticMesh (e.g., `/Engine/BasicShapes/Cube`); paper-trail predicts pass. **Latent papercut** (not blocking): `GetNumVertices(int32 LODIndex)` and `GetNumTriangles(int32 LODIndex)` take an `LODIndex` parameter the dispatch passes empty `parameters: {}` for; RC default-init to 0 is the de-facto contract — adding `parameters: { LODIndex: 0 }` explicitly is a defensive cleanup tracked for future hardening, not load-bearing.
