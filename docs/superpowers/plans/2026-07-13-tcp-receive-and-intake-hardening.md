...
Live execution on 2026-07-14 disproved the original probe 14 first response chunk assumption. Windows kernel send buffering could accept the entire 1.21 MB AnimGraph response before Node received its first chunk, leaving no outstanding server write and therefore no `tcp_send_failure`. The corrected implementation holds the final request byte after a bounded prefix-settle interval, sends it, and resets on the next timer turn. On the verified Windows loopback runs this schedule produced one payload-free `tcp_send_failure` with `bytesSent=0`, a response larger than 64 KiB, `reason=send_error`, and `SE_ECONNRESET`/26 before the recovery ping. This is not a platform-independent scheduling guarantee; other TCP stacks remain outside the executed proof boundary.
