...
Live execution on 2026-07-14 disproved the original probe 14 first response chunk assumption. Windows kernel send buffering could accept the entire 1.21 MB AnimGraph response before Node received its first chunk, leaving no outstanding server write and therefore no `tcp_send_failure`. The corrected implementation holds the final request byte after a bounded prefix-settle interval, sends it, and resets on the next timer turn. That ordering makes the server wait for a complete request before dispatch and places the reset before response transmission while preserving the approved one-warning, payload-free log contract and final recovery ping.
