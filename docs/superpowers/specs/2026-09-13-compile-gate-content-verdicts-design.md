...
- If the hashes differ, the mismatch is definitive — see the 2026-09-14 rule-order amendment below. The mtime rules apply only when the hash is unknown.
...
**Amendment (2026-09-14, rule order):** A known mismatch (`contentIdentical === false`) is definitive on its own — it is not equivalent to "fall through to the mtime rules unchanged" as first written above. `sync-plugin.bat` copies more than `Source/**`: the digest also covers `Resources/` and `UEMCP.uplugin`, but the mtime rules only ever look at `Source/` mtimes. A target whose deployed `Resources/` differs from the repo while every `Source/` mtime happens to read fresh could pass the mtime rules and report `SYNC` — a confident wrong answer for content already proven to differ. And since a build cannot fix content that differs from the repo, `NEEDS-BUILD` is equally wrong once the mismatch is known. The rule now decides on its own: `NEEDS-DEPLOY` with reason `deployed content differs from repo and the DLL predates the repo source` when the DLL is older than the repo source; otherwise `NEEDS-SYNC` with reason `deployed content differs from repo (hash mismatch); timestamps alone looked current` when the mtime rules would have read `SYNC` or `NEEDS-BUILD` (both wrong here), or the mtime rules' own `NEEDS-SYNC`/`NEEDS-DEPLOY` verdict and reason, unchanged, when they already agree. The mtime rules remain the sole fallback only for `contentIdentical === null` — an unknown hash, not a known mismatch — because the digest is authoritative: it covers strictly more of the tree than the mtime scan does, so a definitive mismatch it reports can never be second-guessed by timestamps that see less.
...
- `test-verify-deploy.mjs`: the content rules (identical + DLL after sync → SYNC; identical + DLL before sync → NEEDS-BUILD; identical + no DLL → never-built rules; hashes differ → definitive NEEDS-DEPLOY or NEEDS-SYNC per the 2026-09-14 rule-order amendment, never SYNC or NEEDS-BUILD; missing hashes → mtime rules); `--json` document shape, error document shape, exit code parity with text mode (drive the printer through its existing seam or a small extracted `buildJsonReport(targets)` function).
