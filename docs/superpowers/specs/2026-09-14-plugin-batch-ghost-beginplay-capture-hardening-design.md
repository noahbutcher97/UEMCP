...
## Amendment (2026-09-15): the timer site was never affected

§2's first fact and §4.1 claimed that a timer chain inserted under a reused `ReceiveBeginPlay` compiled to nothing whenever that node was the editor's auto-placed disabled ghost, and the approved fix was to call `SetEnabledState(ENodeEnabledState::Enabled, /*bUserAction=*/true)` on it directly. Task 2's red step falsified the premise for `add_blueprint_timer`: with `EnsureEventNodeEnabled` stubbed out to a no-op, the node the handler produced was already enabled and the compiled class already implemented `ReceiveBeginPlay`. The reason is in the engine's pin-linking path, not in anything the handler does: `UEdGraphPin::MakeLinkTo` (`Runtime/Engine/Private/EdGraph/EdGraphPin.cpp:535-536`) calls `ConvertConnectedGhostNodesToRealNodes` on both pins it links, and that function (`:1804-1824`) already does `SetEnabledState(ENodeEnabledState::Enabled, /*bUserAction=*/false)` on a ghost node it connects, clearing `NodeComment` and hiding the comment bubble. `add_blueprint_timer` links its timer node to `ReceiveBeginPlay`'s `Then` pin before returning, so the ghost was always converted by the engine's own linking, independent of anything the plugin does about enabled state.

The defect is real, but narrower than §2 and §4.1 described: it is at the two reuse sites that return the found `ReceiveBeginPlay` (or the overridden parent event) without linking anything to it — `HandleAddBlueprintEventNode`'s dedup path and `HandleOverrideBlueprintParentMember`'s reuse path. There, no pin ever links to the ghost, `MakeLinkTo` never runs, and the node the caller receives stays disabled; the compiled class has no `ReceiveBeginPlay` or `ReceiveTick` stub until something else links to it. Task 3's red step measured this directly: the `EventNodeGhostSites` native test failed eight assertions before the two call sites were changed to call the enabling helper, and passed after.

The resulting helper (`EnsureEventNodeEnabled` in `Public/BlueprintHandlerHelpers.h`) replicates the engine's own conversion rather than performing a user-action enable: `SetEnabledState(ENodeEnabledState::Enabled, /*bUserAction=*/false)` plus clearing the ghost comment, matching `ConvertConnectedGhostNodesToRealNodes` statement-for-statement since the engine function itself is private and cannot be called directly. It is called at all three reuse sites, including `add_blueprint_timer`'s, for a uniform `enabled_ghost: true` report; at the timer site the call sits inside `FindOrCreateReceiveBeginPlay`, which returns to the caller before `TryLinkPins`/`MakeLinkTo` runs, so the helper is what actually converts the ghost and its return value is what `enabled_ghost` reports — `MakeLinkTo`'s own conversion, moments later when the timer's exec pin links to the node, is the one that becomes a no-op, since the node is no longer `IsAutomaticallyPlacedGhostNode()` by then. Task 2's red step still passed with the helper stubbed because `MakeLinkTo` backstops it at this one site; with the helper live, `add_blueprint_timer` reporting `enabled_ghost: true` is a direct report of what the helper found and changed, not an inference about which code path acted.

§8's wire-change list and the D201 row name only `get_viewport_screenshot` for the relocation of relative capture output paths under `Saved/UEMCP/Captures/`. The shared `ResolveCaptureOutputPath` resolver applies the same rule to all three capture tools: a relative `out_png` for `capture_asset_editor` and `capture_pie_viewport` previously landed directly under `Saved/` through the old `FinishCapture`, and now joins `Saved/UEMCP/Captures/` like `get_viewport_screenshot` does. The wire-change record is widened accordingly; no other behavior described in §8 changes.
