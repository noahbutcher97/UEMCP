...
### BUG-2 — Three pre-existing quirks surfaced by the WS5a handler tests (not fixed there: all three change wire behaviour)
...
- **Trigger**: the next change to `BlueprintHandlers.cpp` pin or literal handling, with a native test for each.
