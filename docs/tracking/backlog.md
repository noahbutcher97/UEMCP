...
In-flight as of 2026-04-23 (M-enhance Sessions 1-3 landed; Session 4 tight + bounded):

- **M-enhance** — handoff `docs/handoffs/m-enhance-hybrid-transport.md`. **Sessions 1-3 shipped** per D74 + D75 + D76: 35 agent-facing tools + 14 plugin C++ handlers + transport infra + save-hook. Test baseline 1037 → 1196 passing. **Session 4 remaining (0.5-1 session, well-bounded)**: 3F-4 production commandlet (batch sidecar generator — Oracle-A promotion); Editor-menu "Regenerate UEMCP sidecar" (Slate FUICommandList); Visual capture plugin handler (`get_asset_preview_render` — FPreviewScene + FWidgetRenderer); additional TOOLSET_TIPS for animation / data-assets / input-and-pie / editor-utility / asset-registry toolsets; integration smoke test (user action).

**User-action items pending** (can happen before, during, or after Session 4 dispatch — orthogonal):
- **Integration smoke test (expanded scope post-Session 3)**: close editor → `sync-plugin.bat "D:/UnrealProjects/5.6/ProjectA/ProjectA/ProjectA.uproject"` → `Build.bat` → open editor → verify 5 NEW handlers from Sessions 2-3 (get_struct_reflection, get_datatable_contents, get_string_table_contents, list_data_asset_types, regenerate_sidecar) + confirm OnObjectPreSave save-hook auto-writes sidecar on next BP save. CP3's 10 original handlers from yesterday's smoke test already covered. If RC plugin doesn't auto-enable via UEMCP.uplugin dep chain, add `{"Name": "RemoteControl", "Enabled": true}` to ProjectA.uproject Plugins[].
- **Cross-transport transaction semantics** (FA-ε §Open 3) + **PIE teardown race** (M-enhance handoff §Biggest-unknowns 4) — both remain untested; require live editor runs during integration smoke.
