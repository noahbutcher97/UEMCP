...
- **D188 follow-on**: `test-s-b-base-differential.mjs` and `test-uasset-parser.mjs` still hardcode `/Game/Actors/...` probe paths — migrate to `findContentAsset` discovery when next touched, since they will re-stale on the next content reorg the same way `test-phase1.mjs`/`test-verb-surface.mjs` did before their D188 migration.
