...
In-flight as of 2026-04-22 (M-enhance Session 2 landed; Session 3 pending):

- **M-enhance** — handoff `docs/handoffs/m-enhance-hybrid-transport.md`. **Sessions 1-2 shipped** per D74 + D75: CP1 transport infra (`12b1a13`), CP2 FULL-RC 11 tools (`7e91e1d`), CP3 plugin C++ 10 handlers (`ca479f7`), CP4-partial 10 FULL-TCP JS handlers (`1e811b7`). Test baseline 1037 → 1129 passing on M-enhance-owned files. **Remaining for Session 3**: 13 PARTIAL-RC hybrid-dispatch tools, visual capture + struct_definition + bp_functions plugin handlers, CP5 narrow sidecar + save-hook + 3F-4 commandlet, CP6 yaml polish + TOOLSET_TIPS + final rotation.

**User-action items pending** (can happen before, during, or after Session 3 dispatch — orthogonal):
- **Integration smoke test**: close editor → `sync-plugin.bat "D:/UnrealProjects/5.6/ProjectA/ProjectA/ProjectA.uproject"` → `Build.bat ProjectAEditor Win64 Development -project=D:/UnrealProjects/5.6/ProjectA/ProjectA/ProjectA.uproject -WaitMutex` → open editor → verify RC plugin loads + per-handler smoke test. If RC plugin doesn't auto-enable via UEMCP.uplugin dep chain, add `{"Name": "RemoteControl", "Enabled": true}` to ProjectA.uproject Plugins[].
- **Fixture drift regen (BP_OSControlPoint)**: 8 pre-existing fixture-drift failures on `test-uasset-parser.mjs` (3) + `test-s-b-base-differential.mjs` (5). Noah edited BP_OSControlPoint between Sessions 1-2 (parser sees 223 exports / 398 edges vs oracle's stale 182 / 330). Fix: close editor → re-run DumpBPGraph commandlet against BP_OSControlPoint to regenerate oracle JSON. Recipe in `plugin/UEMCP/Source/UEMCP/Private/Commandlets/fixtures/fixtures.txt`.
