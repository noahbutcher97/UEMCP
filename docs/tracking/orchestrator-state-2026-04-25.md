...
## Recent D-log highlights (D77 → D104)
...
| **M5-animation+materials** | ⏸ Dispatchable now — handoff drafted with M5-PREP patterns inherited. Scope: 7 not-shipped (4 anim + 3 materials). `set_material_parameter` ships as RC delegate per D101 (ii); `get_audio_asset_info` SUPERSEDED-vs-implement decision per D101 (v) is sub-worker's call. | 1.5-2 sessions |
| **M5-input+geometry** | ⏸ Dispatchable now — handoff drafted. Scope: 6 not-shipped (3 Enhanced Input + 3 procedural mesh). **Noah action gating**: confirm Geometry Script plugin enabled in Project A before dispatch per D101 (iii). | 1-1.5 sessions |
| **M5-editor-utility** | ⏸ Dispatchable now — handoff drafted with security-sensitive scope. Scope: 6 not-shipped (Python exec + asset delete). `run_python_command` ships with deny-list + `--enable-python-exec` startup flag + per-call log per D101 (iv). | 1.5-2 sessions |
| **.githooks pre-commit/pre-push missing-file warning** | ⏸ Tiny improvement — D103 finding #2 surfaced that fresh-clone / worktree contexts can have `.git/info/forbidden-tokens` absent → silent gate bypass. Add WARNING (not block) when file missing | 0.1 session; bundle with next CLAUDE.md/.githooks pass |
...
| **Rotation-runner harness improvement** | ⏸ Tiny — D104 surfaced "silent-zero test failures" as the worst regression class. Add fail-loud detection for import errors in test files so future barrel-deletion refactors don't silently drop assertions | 0.25-0.5 session |
